...
exports.run = (client, message, args) => {
    if(!args){
     ...
     message.channel.send("Starting compiler thread....").then((msg) => {
         ...
         .replace("python", "")
         .trim()
        fs.writeFile('tmp.js', str , function (err) {
        if (err) throw err;
            console.log('[JSEXE] TmpFile is created successfully.');
        });  
        message.channel.send("Attempting to run. If no output is sent, please, check your code.")
        const python = spawn('node', ['tmp.js']);
        python.stdout.on('data', function (data) {
                dataToSend = data.toString();
                try{const out = new Discord.MessageEmbed() 
                    ...
                    try {
                        ...
                        return console.log('[JSEXE] TmpFile is deleted successfully.');
                      } catch(err) {
                        console.error(err)
